...
token_rotation_state = {
    "BD": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50},
    "IND": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50},
    "BR": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50},
    "US": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50},
    "SAC": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50},
    "NA": {"last_batch": -1, "total_batches": 0, "tokens_per_batch": 50}
}

# --- FORCE BD TOKENS & URL (ALL REGIONS WILL USE BD) ---
FORCE_BD_TOKENS = True   # সব রিজনের জন্য BD টোকেন ব্যবহার করবে
FORCE_BD_URL = True      # সব রিকোয়েস্ট BD URL এ পাঠাবে
...
def fetch_regional_endpoint(region_id):
    # সব রিজনের জন্য BD endpoint ব্যবহার করবে যদি FORCE_BD_URL True হয়
    if FORCE_BD_URL:
        return "https://clientbp.ggpolarbear.com"
    
    if region_id == "IND": 
        return "https://client.ind.freefiremobile.com"
    elif region_id in ["BR", "US", "SAC", "NA"]: 
...
def load_tokens(server_name):
    """
    সব রিজনের জন্য শুধুমাত্র token_bd.json ফাইল ব্যবহার করবে
    ইউজার যা-ই ইনপুট দিক না কেন
    """
    try:
        if FORCE_BD_TOKENS:
            # সব রিজনের জন্য শুধুমাত্র BD টোকেন ফাইল
            with open("token_bd.json", "r") as f:
                tokens = json.load(f)
            app.logger.info(f"Loading tokens from token_bd.json for server: {server_name} (forced BD)")
        else:
            # Original logic (if FORCE_BD_TOKENS is False)
            if server_name == "IND":
...
def get_next_batch_tokens(server_name, tokens_per_batch=50):
    """
    প্রতি ব্যাচে ৫০টি টোকেন দেয়।
    যদি শেষ ব্যাচে টোকেন কম হয়, তাহলে প্রথম থেকে টোকেন নিয়ে ৫০টি পূর্ণ করে।
    """
    ...
    current_batch = token_rotation_state[server_name]["last_batch"] + 1
    
    # Calculate start position for this batch
    start_pos = (current_batch * tokens_per_batch) % total_tokens
    
    # Select tokens with wrap-around
    batch_tokens = []
    for i in range(tokens_per_batch):
        idx = (start_pos + i) % total_tokens
        batch_tokens.append(tokens[idx])
    
    # Update state
    token_rotation_state[server_name]["last_batch"] = current_batch
    token_rotation_state[server_name]["total_batches"] = (total_tokens + tokens_per_batch - 1) // tokens_per_batch
    
    # Calculate token range display
    end_pos = (start_pos + tokens_per_batch - 1) % total_tokens
    if start_pos <= end_pos:
        range_display = f"{start_pos + 1}-{end_pos + 1}"
    else:
        range_display = f"{start_pos + 1}-{total_tokens}, 1-{end_pos + 1}"
    
    return batch_tokens, current_batch, token_rotation_state[server_name]["total_batches"], range_display
...
async def send_multiple_requests_normal(uid, server_name, url):
    """Normal like method - sends likes using ALL tokens (each token sends 1 like)"""
    try:
        region = server_name
        ...
        # প্রতিটি টোকেন থেকে ১টি করে লাইক - মোট যত টোকেন তত লাইক
        tasks = []
        ...
        app.logger.error(f"Exception in send_multiple_requests_normal: {e}")
        return None

async def send_multiple_requests_rotated(uid, server_name, url, batch_tokens):
    """Rotated like method - sends likes using the provided batch of tokens (50 likes per batch)"""
    try:
        region = server_name
        protobuf_message = create_protobuf_message(uid, region)
        if protobuf_message is None:
        ...
        app.logger.error(f"Exception in send_multiple_requests_rotated: {e}")
        return None
...
def make_request(encrypt, server_name, token):
    try:
        # সব রিজনের জন্য BD URL ব্যবহার করবে যদি FORCE_BD_URL True হয়
        if FORCE_BD_URL:
...
def fetch_player_info(uid):
    try:
        url = f"https://zx-infoapi.onrender.com/player-info?uid={uid}"
...
def serve_api_docs():
    ...
    <body>
        ...
        <div class="endpoint-block">
            ...
            <pre><code>yourdomain.com:25599/token?access_token=YOUR_GARENA_ACCESS_TOKEN</code></pre>
        </div>
        ...
        <div class="endpoint-block endpoint-gift">
            ...
            <pre><code>yourdomain.com:25599/Gift</code></pre>
        </div>

        <div class="endpoint-block endpoint-like">
            <h3><span class="badge badge-like">GET</span> /likex</h3>
            <p>Profile Like Booster (Full Mode) - Sends likes using ALL available tokens. If you have 500 tokens, sends 500 likes.</p>
            <p><strong>Example Syntax:</strong></p>
            <pre><code>yourdomain.com:25599/likex?uid=123456789&server_name=BD</code></pre>
        </div>

        <div class="endpoint-block endpoint-like">
            <h3><span class="badge badge-like">GET</span> /jobayarx</h3>
            <p>Profile Like Booster (Batch Mode) - Sends 50 likes per request. Auto-rotates through tokens. If last batch has less than 50 tokens, takes remaining from beginning.</p>
            <p><strong>Example Syntax:</strong></p>
            <pre><code>yourdomain.com:25599/jobayarx?uid=123456789&server_name=BD</code></pre>
        </div>
        
        <div class="endpoint-block">
            <h3><span class="badge badge-get">GET</span> /reset_rotation</h3>
            <p>Reset token rotation state for a server.</p>
            <p><strong>Example Syntax:</strong></p>
            <pre><code>yourdomain.com:25599/reset_rotation?server_name=BD</code></pre>
        </div>
    </body>
    </html>
    """
            ...
            if res.status_code == 200:
                ...
                for proto_item in dict_converted.get('items', []):
                    ...
                    if diamonds > 0 and coins > 0: 
                        price_lbl = f"💎 {diamonds} / 🪙 {coins}"
                    elif diamonds > 0: 
                        price_lbl = f"💎 {diamonds}"
                    elif coins > 0: 
                        price_lbl = f"🪙 {coins}"
                ...
                return jsonify({"success": False, "message": "Garena Core Server Refused Response Code"}), 400
        except Exception as err: 
            return jsonify({"success": False, "message": f"Fatal Core Exception: {str(err)}"}), 500
...
def pipe_gift_transmission():
    ...
    if not region: 
        return jsonify({"success": False, "message": "EXPLOIT DENIED: TOKEN AUTH DISCREPANCY"}), 400
    ...
    try:
        ...
        if res.status_code == 200:
            if jwt in ZIBON_SYSTEM_CACHE: 
                del ZIBON_SYSTEM_CACHE[jwt]
            return jsonify({"success": True, "message": f"✓ SUCCESS: Packet injected. Gift transmitted to UID: {target_uid}"})
        else:
            try: 
                return_error = res.content.decode('utf-8').strip()
            except Exception: 
                return_error = f"HTTP STATUS CODE: {res.status_code}"
...
@app.route('/likex', methods=['GET'])
def handle_like_normal():
    """
    NORMAL MODE - FULL TOKEN USAGE
    যত টোকেন থাকবে তত লাইক দিবে (সব টোকেন ব্যবহার করবে)
    ইউজার যা-ই server_name দিক না কেন, BD টোকেন এবং BD URL ব্যবহার করবে
    """
    ...
    try:
        def process_request():
            ...
            # সব রিজনের জন্য BD ব্যবহার করবে (ইউজার যা-ই দিক না কেন)
            server_name_used = "BD"
            ...
            before = make_request(encrypted_uid, server_name_used, check_token)
            if before is None:
            ...
            # সব রিজনের জন্য BD URL ব্যবহার করবে
            url = "https://clientbp.ggpolarbear.com/LikeProfile"

            # সব টোকেন ব্যবহার করে লাইক দিবে
            asyncio.run(send_multiple_requests_normal(uid, server_name_used, url))

            # Get after like count
            after = make_request(encrypted_uid, server_name_used, check_token)
            if after is None:
            ...
            try:
                ...
                raise Exception(f"Error parsing after data: {e}")
            ...
            result = {
                ...
                "expected_likes": total_tokens,
                "server_name_used": server_name_used,
                "user_provided_server": server_name
            }
            return result
        ...
        app.logger.error(f"Error processing likex request: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/jobayarx', methods=['GET'])
def handle_like_rotated():
    """
    BATCH MODE - 50 LIKES PER REQUEST
    প্রতি কলেই ৫০টি লাইক দিবে (শেষ ব্যাচে কম থাকলে প্রথম থেকে নিয়ে ৫০ পূর্ণ করবে)
    ইউজার যা-ই server_name দিক না কেন, BD টোকেন এবং BD URL ব্যবহার করবে
    """
    uid = request.args.get("uid")
    server_name = request.args.get("server_name", "").upper()
    ...
    try:
        def process_request():
            ...
            # Get next batch of 50 tokens (auto rotation with wrap-around)
            batch_tokens, batch_num, total_batches, token_range = get_next_batch_tokens(server_name_used, tokens_per_batch=50)
            
            if not batch_tokens or len(batch_tokens) == 0:
                raise Exception("No tokens available for this server.")
            
            tokens_used = len(batch_tokens)
            check_token = batch_tokens[0]['token']
            encrypted_uid = enc(uid)
            if encrypted_uid is None:
                raise Exception("Encryption of UID failed.")
            ...
            # Send 50 likes using this batch
            asyncio.run(send_multiple_requests_rotated(uid, server_name_used, url, batch_tokens))
            ...
            result = {
                ...
                "tokens_used_in_this_batch": tokens_used,
                "token_range": token_range,
                "total_batches_possible": total_batches,
        ...
        app.logger.error(f"Error processing jobayarx request: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/reset_rotation', methods=['GET'])
def reset_rotation():
    """Reset token rotation state for a specific server"""
    server_name = request.args.get("server_name", "").upper()
    
    if not server_name:
        return jsonify({"error": "server_name is required"}), 400
    
    if server_name in token_rotation_state:
        token_rotation_state[server_name]["last_batch"] = -1
        return jsonify({
            "success": True,
            "message": f"Rotation state reset for server {server_name}",
            "server": server_name,
            "state": token_rotation_state[server_name]
        })
    else:
        return jsonify({
            "success": False,
            "error": f"Invalid server name. Valid servers: {list(token_rotation_state.keys())}"
        }), 400
